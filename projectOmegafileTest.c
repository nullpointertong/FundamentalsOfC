#include "projectHeaderOmegafileTest.h.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
        ...
        switch (input) {
            ...
            case ('0'):
                ...
                /*if (writeFile(linkList, numUsers) == 1) {
                    printf("good\n");
                } else if (writeFile(linkList, numUsers) == 0) {
                    printf("bad\n");
                }*/
                /*writeFile(linkList, numUsers);*/
                
               if(writeFileV2(linkList, numUsers)==1)
                    printf("yay!!");
                else printf("nay");
                return 0;
               }
...
int withdrawMoney(node_t p) {
   ...
   /* if(miniMenu("Withdraw Money", p, linkList) == '1')
    {*/
        ...
        /*fgets(accountID1, strlen(p->user.username)+1, stdin);*/
        scanf("%s", accountID1);
        ...
        for (j = 0; j < 6; j++) {
            ...
            if ((strcmp(p->user.account[j].accountID,
                        accountID1)) == 0) { /*search for account */
...
int writeFile(node_t linkList, int* numUsers)
{
    ...
    for (i = linkList;  i!= NULL; i = i->nextp)
        ...
        fprintf(fp, "%s %s %s %d ", i->user.userID, i->user.username,
                i->user.password, i->user.numAccounts);
        /*numAccounts used to determine how many accounts to print*/
        
        for (count = 0; count < i->user.numAccounts; ) {
            fprintf(fp, "%s %lf %d", i->user.account[count].accountID,
                    i->user.account[count].accountValue, i->user.account[count].availableFlag);
...
int writeFileV2(node_t linkList, int* numUsers)
{
    ...
    printf("pass:%s", linkList->nextp->user.password);
    fp=fopen("Database.txt", "w"); /*locates and opens/creates database file*/
